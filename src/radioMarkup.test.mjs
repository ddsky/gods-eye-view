...
test('no unchanged Realtime tool definition drifts silently', () => {
  ...
  const digest = createHash('sha256')
    ...
    .digest('hex')
    .slice(0, 16);
  // ALPR intentionally extends the two layer enums, and World News extends
  // those plus analyst_query's layer enum and filter fields; retain the
  // complete pin.
  assert.equal(digest, 'e99b5655fad84959', 'an unchanged Realtime tool definition drifted');
});
