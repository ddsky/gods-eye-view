...
test('the voice TOOL SCHEMA matches the pinned release — the mission mapping is instructions only', () => {
  ...
  // World News adds its ID to the same two menus, the analyst_query layer enum
  // and that tool's filter-field description.
  ...
  assert.equal(block.length, 26303, 'serialized tool schema length drifted');
  assert.equal(
    crypto.createHash('sha256').update(block).digest('hex'),
    '52a9d3821bf896b3d94e13bf7df8853062c499e0467c54ada800b2417ec71fd3',
